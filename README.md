...
- **Hiders (прячущиеся)** — стартуют полностью белыми. Их задача: свободно
  ...
  художник по холсту, — а затем замереть в позе, имитируя часть интерьера. Во
  время поиска у каждого Hider работает свисток: автоматический раз в 45
  секунд или вручную по желанию игрока — звуковая подсказка искателям.
...
│   ├── FreezeService.lua       — заморозка / поза (только Hiders, только фаза Hiding)
