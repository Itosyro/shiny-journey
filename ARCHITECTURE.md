...
- **Лобби** (X от -60 до -18) - открытая зона с 6 `SpawnLocation`
  (`Neutral = true`, работают независимо от команды игрока) и парой скамеек.
...
- **`SeekerWaitingRoom`** (именной Part-контракт, см. `DECISIONS.md`, п.9) -
  отдельная запертая комната без дверей, далеко в стороне (Z=150), доступна
  только телепортом из `RoundManager.teleportPlayersTo`.
...
## Лобби-платформа (ЗАПЛАНИРОВАНО, ещё не реализовано — см. `MEGA_PLAN.md` Часть 1)

Архитектурный слепок будущего лобби, чтобы Sonnet встраивал его в
существующую систему, а не изобретал заново. Ключевой принцип
(ponytail): **отдельного `LobbyService` НЕ будет** — логика ложится на
существующих владельцев.

- **Геометрия** — в `MapBuilder.lua` (функция `buildLobbyPlatform` вместо
  `buildSeekerWaitingRoom`): парящий круглый `Part`-цилиндр высоко над
  картой; в центре невидимый маркер-объём `SeekerVolunteerZone` (встал =
  доброволец-Seeker), по краю 4 маркера `HiderGateZone` (встал =
  доброволец-Hider), остальная площадь = случайное распределение.
  Маркер-контракт `SeekerWaitingRoom` переезжает в центр платформы —
  строка телепорта в `RoundManager` не меняется. Новые маркеры-контракты:
  `HiderSpawn`×N (куда прячутся Hiders на карте), `SeekerSpawn`×N (куда
  материализуются Seekers), `SpectatorSpawn` (уже есть).
- **Выбор роли** — в `PlayerRoleService.AssignRoles`: новая
  `getRoleIntent(player)` читает позицию игрока ОДИН раз в момент раздачи
  (не следит циклом — правило производительности №1) и относит его к
  пулу volunteers/hiderWish/randomPool; слоты Seeker заполняются по
  приоритету. Сигнатура `AssignRoles` не меняется.
- **Разрешения фаз** — `PaintService`/`FreezeService` расширяют проверку
  фазы на `Lobby` (тренировка кисти/поз в лобби) и позже на `Seeking`
  (докраска, MEGA_PLAN 3.2/Q2). Гейтинг клиентских панелей
  (`PaintClient`/`FreezeClient`) зеркалит это.
- **Персистентность покраски** — БЕЗ новых структур: мазки живут на
  частях персонажа. Hider на старте пряток проходит через существующий
  `ResetForNewRound` (стирается), Seeker не проходит (сохраняется).
- **Телепорт-эффект** — локальная `teleportWithEffect` внутри
  `RoundManager` (частицы `ParticleEmitter` + твин прозрачности +
  одновременный телепорт всех Seekers); клиентского кода ноль (сервер
  реплицирует эффект сам).
