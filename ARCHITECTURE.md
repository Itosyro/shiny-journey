...
- **Холл** (X от -60 до -18, `buildEntranceHall`, раньше называлась
  `buildLobby`) - обычная игровая зона здания с парой скамеек, 2
  маркерами `HiderSpawn` и 3 маркерами `SeekerSpawn` (см.
  `teleportWithEffect`, MEGA_PLAN 1.6). Спавнов игроков тут больше нет - они переехали
...
- **Персистентность покраски (проверка, не правка)** - БЕЗ новых структур:
  ...
  (сохраняется).
- **Телепорт-эффект (готово)** - локальная `teleportWithEffect` внутри
  `RoundManager`: одним проходом по всем Seekers - `ParticleEmitter`
  (встроенная искра, без ассета) + твин прозрачности до 1, один общий
  `task.wait(GameConfig.TELEPORT_EFFECT_SECONDS)`, `teleportPlayersToRandomOf`
  на маркер `SeekerSpawn`×3 (`MapBuilder.buildEntranceHall`), обратный
  твин + вторая вспышка. Клиентского кода ноль - сервер реплицирует эффект
  сам. Вызывается в начале `runSeekingPhase`. Попутно закрыт латентный
  пробел: раньше Seekers физически никогда не покидали запертую комнату
